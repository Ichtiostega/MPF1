...
class heat_model:
    ...
    def analitic(self, z, t, m):
        return self.month_mean_temp[m] + self.month_amplitude(m) * math.sin(2*math.pi*t/day - z/self.D) * math.e**(-z/self.D)
    ...
    def plot_layer_heat_progression(self, ax, label=None, color=None):
        ax.plot(np.arange(0, 3 + self.z_step, self.z_step), self.earth, label=label, color=color)
        return ax
...
def zad8():
    ...
    for i in range(steps[-1]):
        m.iterate_month(12-len(steps))
        if i % plot_step == 0:
            m.plot_layer_heat_progression(ax, color=clr.hsv_to_rgb([(12-len(steps))/12,1,1]))
...
def zad9():
    ...
    while not stable:
        ...
        for j in range(steps[-1]):
            model.iterate_month(12-len(steps))
            tmp += model.earth
